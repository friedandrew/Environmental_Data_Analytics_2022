...
> The ozone levels are withing the safe levels
...
Density3 <-ggplot( EPAair, aes(x = Ozone, fill = as.factor(Year)))+
...
O3Regression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(O3Regression)
...
epa.na<- EPAair%>%
  na.omit()
epaLMAIC <- lm(data = epa.na, PM2.5 ~ Ozone + meanLat + meanLng + Month + Year)
step(epaLMAIC)
...
Ozonebestregression<- lm(formula = PM2.5 ~ Ozone + meanLat + meanLng + Month, data = epa.na)
