...
plot1_Peter_Paul<- ggplot(subset(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C, color = year4 ))+
  geom_point()+
...
PP.subset<- subset(PeterPaul.chem.nutrients, depth == 0)
PP.subset
#2. 
...
plot2_Peter_Paul<- ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth ))+
  geom_point(size = 0.5)+
...
plot3_EPA <- ggplot(EPAair, aes(x = PM2.5, y = Ozone, color = SITE_LATITUDE))+
