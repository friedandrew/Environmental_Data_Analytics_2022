...
```{r}
#7
EPAair_Combined_O3_PM25<- rbind(EPAair_O3_NC2018_Processed, EPAair_O3_NC2019_Processed, EPAair_PM25_NC2018_Processed, EPAair_PM25_NC2019_Processed)

#8
EPAair_Combined_O3_PM25_processed <- EPAair_Combined_O3_PM25%>%
...
#9
EPAair_Combined_O3_PM25_processed_pivot <- pivot_wider(EPAair_Combined_O3_PM25_processed, names_from = AQS_PARAMETER_DESC, values_from = DAILY_AQI_VALUE)

#10
dim(EPAair_Combined_O3_PM25_processed_pivot)

#11
write.csv(EPAair_Combined_O3_PM25_processed_pivot, row.names = FALSE, file = "../Data/Processed/EPAair_O3_PM25_NC2122_Processed.csv")
...
```{r}
#12(a,b)
EPAair_Combined_O3_PM25_processed_pivot_summary <- EPAair_Combined_O3_PM25_processed_pivot%>%
  filter(!is.na(Ozone))%>%
  filter(!is.na(PM2.5))%>%
  group_by(Site.Name, Month, Year)%>%
  summarise(Average_O3 = mean(Ozone), 
            Average_PM25 = mean(PM2.5))

#13
dim(EPAair_Combined_O3_PM25_processed_pivot_summary)

```

14. Why did we use the function `drop_na` rather than `na.omit`?

> Answer: I used !is.na() to omit all of the NA values for each column.
