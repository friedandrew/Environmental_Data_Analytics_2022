...
NTL.phys.data.Tuesday.90.99 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: row number is not a value that is in a cell in the dataset, so filter cannot find it. Also the row number of observations can be changed.
...
For maximum reproducability. Then you can run the analysis code on any tidy dataset.
