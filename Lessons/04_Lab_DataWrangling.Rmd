...
NTL.phys.data.Tuesday.1990.1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999), lakename == "Tuesday Lake")
...
NTL.phys.data.Tuesday.1990.1999.July.avg.surface.temp<- 
  NTL.phys.data.Tuesday.1990.1999.July%>%
  filter(depth == 0)%>%
...
NTL.phys.data.irradiance.pivot.gathered<- NTL.phys.data%>%
  pivot_longer(irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "radiation" )

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.phys.data.irradiance.pivot.spread<- pivot_wider(NTL.phys.data, names_from = "depth", values_from = "temperature_C" )
