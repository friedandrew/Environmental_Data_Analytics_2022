...
> Answer: Data frames and matricies are stored as "data", and vectors are stored as "values"

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: It autocompletes the columns
...
```{r}
highsF<- c(49,54, 39,40,45,38,44,51,52,52)
lowsF<- c(30,28,16,27,26,19,22,26,26,28)
```
...
```{r}
tempconvertFtC<- function(x){
  y<- (x-32)*5/9
  return(y)
}
highsC<-tempconvertFtC(highsF)
lowsC<-tempconvertFtC(lowsF)
highsC
lowsC
```
...
```{r}
temperature.data.frame<- data.frame(highsF,highsC,lowsF,lowsC)
names(temperature.data.frame)<- c("High in F", "Low in F", "High in C", "Low in C")
```
...
```{r}
summary(temperature.data.frame)
summary(temperature.data.frame$`High in F`)
sd(temperature.data.frame)
sd(temperature.data.frame$`High in F`)
```
