...
WIndEnergy.Summary <- WindEnergy %>% 
  mutate(HUC_8 = substr(HUC_12, 1,8)) %>% 
  group_by(HUC_8) %>% 
  summarize(Sum.AvgWindEnergy = sum(AvgWindEnergy),
            Sum.kWhkm2day = sum(kWhkm2day))


#Join to HUC_8 features
huc8_sf_wind <- huc8_sf %>% 
  left_join(WIndEnergy.Summary, by = c("HUC_8" = "HUC_8"))



#View the outputs
mapview(huc8_sf_wind, zcol = "Sum.AvgWindEnergy")
ggplot(huc8_sf_wind)+
  geom_sf(aes(fill = Sum.AvgWindEnergy))
...
counties_sf_utm <- st_transform(counties_sf_utm, crs = 26917)
state_sf_utm <-    st_transform(state_sf_utm, crs = 26917)
...
clipped_HUCss <- selected_HUCS %>% 
  st_intersection(triangle_sf)

mapview(clipped_HUCss)
