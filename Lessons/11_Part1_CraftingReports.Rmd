...
```{r echo=FALSE}
kable(NTL_summary_tn, caption = "Total Nitrogen Summary by Lake Name")
kable(NTL_summary_tp, caption = "Tota Phosphorus Summary by Lake Name")
```
...
```{r echo=FALSE, fig.align='left', fig.cap="Nitorgen Concentrations over Time"}
ggplot(NTL_processed, aes(x = sampledate, y = tn_ug))+
    geom_point(aes(color = lakename), alpha = 0.7)+
    scale_y_log10()+
    labs(x = "Date", y = "Total Nitrogen Concentration (ug)")

```

```{r echo=FALSE, fig.align='left', fig.cap="Phosphorus Concentrations over Time"}
ggplot(NTL_processed, aes(x = sampledate, y = tp_ug))+
    geom_point(aes(color = lakename), alpha = 0.7)+
    scale_y_log10()+
    labs(x = "Date", y = "Total Phosphorous concentration (ug)")
```
...
The dataset shown is from the NTL research program, which is database of chemical parameters sampled in lakes across Wisconsin over a long period of time. The plots shown above graph the concentrations of nitrogen and phosphorus in the lakes over time. These two chemicals are pollutants from agricultural runoff, and show a slight increasing trend over time. 

***
To analyze this dataset further, I would separate out the data by lakes, and see which lakes have a stronger increasing trend in tn and tp over time. Then I would see which other variables measured at each lake also correlate with an increasing trend in these pollutatants.
